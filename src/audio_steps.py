    ...
    def _request():
        ...
        for payload in payload_candidates:
            ...
            if response.status_code < 400:
                ...
                detected_suffix, detected_format = detect_audio_format(content_type, audio_bytes)
                normalized_path = normalize_audio_for_manim(
                    audio_bytes=audio_bytes,
                    source_format=detected_format,
                    target_path=output_path.with_suffix(".wav"),
                )
...
def normalize_audio_for_manim(audio_bytes: bytes, source_format: str, target_path: Path) -> Path:
    target_path = Path(target_path).resolve()

    audio = AudioSegment.from_file(io.BytesIO(audio_bytes), format=source_format)
    if len(audio) < 250:
        raise RuntimeError(f"TTS audio is too short to be valid: {target_path} ({len(audio)} ms)")
    if audio.rms == 0:
        raise RuntimeError(f"TTS audio is silent: {target_path}")
...
def detect_audio_format(content_type: str, audio_bytes: bytes) -> tuple[str, str]:
...
def _extract_step_index_from_call(call: ast.Call, alias_offsets: dict[str, int]) -> int | None:
    if len(call.args) < 2:
        return None
    ...
    for stmt in statements:
        ...
        if isinstance(stmt, ast.Expr) and isinstance(stmt.value, ast.Call) and isinstance(stmt.value.func, ast.Attribute):
            ...
            if attr == "play_synced_step":
                step_index = _extract_step_index_from_call(call, alias_offsets)
